...
class VanillaWordEmbedding(nn.Module):
    ...
    def forward(self, sentence):
        ...
        # STUDENT
        inp = ag.Variable( torch.LongTensor([self.word_to_ix[t] for t in sentence ]))
        for x in inp:
            embeds.append(self.word_embeddings(x))
        # END STUDENT
        return embeds
...
class BiLSTMWordEmbedding(nn.Module):
    ...
    def __init__(self, word_to_ix, word_embedding_dim, hidden_dim, num_layers, dropout):
        ...
        x = int(hidden_dim/2)
        self.lstm = torch.nn.LSTM(input_size=word_embedding_dim,hidden_size=x,num_layers=num_layers,dropout=dropout,bidirectional=True,batch_first=True)
...
class SuffixAndWordEmbedding(nn.Module):
    ...
    def __init__(self, word_to_ix, suff_to_ix, embedding_dim):
        ...
        self.sz = len(word_to_ix)+len(suff_to_ix)
        self.word_embeddings = nn.Embedding(len(word_to_ix),embedding_dim//2)
        self.suff_embeddings = nn.Embedding(len(word_to_ix),embedding_dim//2)
    ...
    def forward(self, sentence):
        ...
        # STUDENT
        inp = ag.Variable( torch.LongTensor([ self.word_to_ix[t] for t in sentence ]))
        inp_suffix = ag.Variable( torch.LongTensor([ self.suff_to_ix[t[-2:len(t)]] for t in sentence ]))
        we = self.word_embeddings(inp)
        se = self.suff_embeddings(inp_suffix)
        for t in sentence:
            inp = ag.Variable( torch.LongTensor([ self.word_to_ix[t]]))
            inp_suffix = ag.Variable( torch.LongTensor([ self.suff_to_ix[t[-2:len(t)]]]))
            we = self.word_embeddings(inp)
            se = self.suff_embeddings(inp_suffix)
            concat = torch.cat([we,se],1).view(1,self.embedding_dim)
...
class FFCombiner(nn.Module):
    ...
    def forward(self, head_embed, modifier_embed):
        ...
        """
        # STUDENT
        embeded = torch.cat([head_embed,modifier_embed], 1).view(1, -1)
        out = F.tanh(self.linear1(embeded))
...
class LSTMCombiner(nn.Module):
    ...
    def forward(self, head_embed, modifier_embed):
        ...
        # STUDENT
        embeded = torch.cat([head_embed,modifier_embed],1).view(-1,1)
        lstm_out, self.hidden = self.lstm(embeded.view(1,1,-1),self.hidden)
        return lstm_out[0]
        # END STUDENT
...
class FFActionChooser(nn.Module):
    ...
    def __init__(self, input_dim):
        ...
        self.linear2 = nn.Linear(input_dim,Actions.NUM_ACTIONS)
        # END STUDENT
    ...
    def forward(self, inputs):
        ...
        out1 = F.relu(self.linear1(embeded))
...
class LSTMActionChooser(nn.Module):
    ...
    def __init__(self, input_dim, num_layers, dropout):
        ...
        #print(input_dim)
        self.lstm_layer = nn.LSTM(input_dim,input_dim,num_layers=num_layers, dropout=dropout)
        self.layer2 = nn.Linear(input_dim,Actions.NUM_ACTIONS)
        # END STUDENT
        self.hidden = self.init_hidden()
    ...
    def forward(self, inputs):
        ...
        # STUDENT
        embeded = torch.cat(inputs,1).view(-1,1)
        lstm_out, self.hidden = self.lstm_layer(embeded.view(1,1,-1),self.hidden)
        out = lstm_out
        #print(out.view(1,12))
        output = F.relu(out.view(1,self.input_dim))
        #print(F.log_softmax(self.layer2(output),dim = 1))
        return F.log_softmax(self.layer2(output),dim = 1)
        # END STUDENT
