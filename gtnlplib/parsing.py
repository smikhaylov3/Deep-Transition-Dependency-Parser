...
class ParserState:
	...
	def done_parsing(self):
		...
		"""
		# STUDENT
		empty =  (self.input_buffer_len() == 1 ) and (self.stack_len()==1)
		return empty
	def stack_len(self):
	...
	def _arc(self, action):
		assert len(self.stack) >= 1, "ERROR: Cannot arc with stack length less than 1"
	...
	def _create_arc(self, head, modifier):
		...
		"""
		#STUDENT
		combinedEmbedding = self.combiner(head.embedding,modifier.embedding)
		self.input_buffer.insert(0,StackEntry(head.headword,head.headword_pos,combinedEmbedding))
		ret = DepGraphEdge((head.headword,head.headword_pos),(modifier.headword,modifier.headword_pos))
		return ret
	...
	def _validate_action(self, action_to_do):
		...
		if action_to_do == Actions.SHIFT:
			if self.input_buffer_len() >2 or self.stack_len() < 1:
				return action_to_do
			else:
...
# END STUDENT
	...
	def _get_arc_components(self, action):
		...
		head = right if action == Actions.ARC_R else left
		mod = left if action == Actions.ARC_R else right
		return  StackEntry(head.headword, head.headword_pos,head.embedding),StackEntry(mod.headword, mod.headword_pos, mod.embedding)
...
class TransitionParser(nn.Module):
	...
	def forward(self, sentence, actions=None):
		...
		while not parser_state.done_parsing():
			...
			opts = [Actions.SHIFT,Actions.ARC_L,Actions.ARC_R]
			if (have_gold_actions):
				gold_act = action_queue.popleft()
				actions_done.append(opts[gold_act])
				if gold_act == 0 :
					parser_state.shift()
				if gold_act == 1:
				...
				prevalid_act = utils.argmax(log_probs)
				#print(prevalid_act)
				act = parser_state._validate_action(prevalid_act)
				#print(act)
				#print('_')
				actions_done.append(opts[act])
				if  act == 0 :
					parser_state.shift()
				if  act == 1:
					edge = parser_state.arc_left()
